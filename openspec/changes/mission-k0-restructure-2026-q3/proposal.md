# Proposal: MISSION K0 結構性重審 (2026 Q3) — 觸發 MISSION「連 2 週落後 → 策略重審」rule

> **觸發原因**：MISSION.md 自身定義的強制升級條件已過期 8 週沒人 fire。
> MISSION 文字：「任一指標連 2 週落後 → 觸發策略重審（不是『再補一輪』）」。
> K0 從 R81 baseline (0/13) 至今 10+ 週，本機 scope 已結構性飽和 (4/13 emit 穩態下限)，
> 4 missing bot (irisx_bot/grokx/lpbot/mimo) OpenAB 端永遠不可達，K40 8/9 + 1 active 9/16 (otel-genai 7 tasks owner M Phase 2/3)。
>
> **R181 觸發訊號鏈**：
> 1. HARNESS「3 連方向偏差 → 強制切換軸」mandate (R181 prompt)
> 2. 策略顧問 #1 行動：「觸發 MISSION 策略重審（不是再補一輪）」(R180)
> 3. MISSION 自身 2-週 lag 觸發條件過期 8 週
>
...
### K0 量化現況（R181 接手 snapshot，奠基 R132 + R144 量測）

| 子指標 | R81 baseline | R144 現況 | 90 天目標 | 差距 | 結構性卡死原因 |
|---|---:|---:|---:|---:|---|
...
### 接力順位卡死鏈（R175-R180 累計 7 條接力清單）

R170 / R173-R180 接力清單 7 條全部 owner M scope 或結構性卡死：
1. K0 Quota 4 missing 補鏈路 (OpenAB scope) ← **本提案直接處理**
2. K0-A1 emit 4/13 → 5/13 護衛 (需 cicx OpenAB 端) ← **本提案直接處理**
3. R117 capsule-brief JS (owner M 5 dirty WIP 之一) ← owner M
4. 護衛過期契約審計延伸 (chain owner M) ← owner M
5. R164 修真 M0 軸延伸 (沒信號) ← 無方向
6. R171 結構性飽和真極限值確認 (已 ship) ← 已 closure
7. R172-R180 transparent 透明化交接軸延伸 ← 結構失靈，**本提案直接處理**

→ 7 條中有 3 條 (1, 2, 7) 必須等本提案決議才能推進。

### 為什麼現在觸發（時機正當性）

- R81 baseline 設定的 90 天驗收期 (~2026-09-04) 還剩 87 天
- 結構性死結確認需時 ~3 週 (R131 → R144 → R150 → R181 共 7 輪量測鎖定)
- 若 Path B (重構) 選，spec-level 提案 + owner M Phase 2/3 + 驗收 90 天 → 87 天夠用 1 個 sprint cycle
- 若 Path A (降級) 選，1 個 MISSION 文字 patch + 1 個護衛 test 落地，**1 輪可 closure** → 立刻 unblock 後續 5 條接力
...
- 列出每條 path 的「降級或升級後續 5 條接力」的影響
- 寫 `tasks.md` 列出 2 條 path 的 task skeleton
- 寫 `design.md` 對 Path B (重構) 給架構草案 (Path A 走純 spec 改, 無 design 需求)
...
- 對齊 `k0-quota-restructure` spec capability
- 量化輸出 3 子指標卡死鏈（emit 4/13 / sample 1/13 / quota 9/13）+ 4 missing 結構性確認 + 7 條接力卡死鏈
- 對應 spec.md KAP-R1-S1: K0 emit 4/13 結構性下限確認
- 對應 spec.md KAP-R1-S2: K0 sample 1/13 結構性下限確認
- 對應 spec.md KAP-R1-S3: K0 quota 4 missing 結構性永久不可達確認
- 驗證: proposal.md 量化表 + R131/R144/R150 量測 chain 完整引述

### MCAP-2: Path A (降級) 設計草案

- 對齊 `k0-quota-restructure` spec capability (Path A subset)
- 對應 spec.md KAP-R2-S1: K0 目標改「本機 4/13 + OpenAB 5/13 + 4 missing 永久非 scope」
- 對應 spec.md KAP-R2-S2: MISSION.md K0 row 量化值 patch (不刪舊值, 加 R181 新欄)
- 對應 spec.md KAP-R2-S3: 護衛 test 1 條 (k0_target_baseline_check.py 對齊 R132 模式)
- 驗證: MISSION.md patch + 護衛 pytest PASS + chain 20→20 守住

### MCAP-3: Path B (重構) 架構草案

- 對齊 `k0-quota-restructure` spec capability (Path B subset)
...
### MCAP-4: 2 path 對後續 5 條接力的影響評估

- 對齊 `k0-quota-restructure` spec capability
- 對應 spec.md KAP-R4-S1: Path A 選 → K0 Quota 4 missing 鏈 (接力 #1) 永久 skip, K0-A1 emit 5/13 (接力 #2) 仍需 OpenAB 端
- 對應 spec.md KAP-R4-S2: Path B 選 → 接力 #1/#2 全部 unblock, R117 capsule-brief (接力 #3) 不受影響, 護衛過期契約審計 (接力 #4) 不受影響
- 對應 spec.md KAP-R4-S3: 7 條接力卡死鏈 → Path B 選後 5 條 unblock, Path A 選後 2 條 unblock
- 驗證: 影響評估表 + owner M 決策 checklist
...
3. **不選風險**：owner M 兩條都不選 → K0 永久 4/13 + 1/13 + 9/13, 接力順位永久卡 5 條, MISSION 失靈
...
- [ ] **Path B** (重構) — 1 sprint closure, unblock 5 條接力, 4 missing 變可達
...
- R175-R180 transparent 透明化軸 7 輪延伸 (結構失靈的真因 = 結構性死結, 非 PUA 不努力)
