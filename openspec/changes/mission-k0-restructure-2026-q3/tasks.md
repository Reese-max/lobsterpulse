...
> PUA (R181) 只 ship proposal.md + tasks.md skeleton + 1 個護衛 test (MCAP-2 對應)。
> Path A / Path B 走 owner M 決策, 不在 R181 scope。
>
> 對應 spec capability: `mission-k0-restructure-2026-q3` (見 proposal.md)。

## Phase 1: Spec closure (R181 PUA scope)
...
- [x] **T-MKR1: 寫 proposal.md** — Goal/Background/Scope/Capabilities 4 段齊
      ...
      驗證: proposal.md 含 4 段 + 3 子指標卡死鏈表 + 4 missing 結構性確認 + 7 條接力卡死鏈 + 2 path 草案 + Decision Asks checklist
...
- [ ] **T-MKRA1: MISSION.md K0 row 量化 patch** — 13/13 → 本機 4/13 + OpenAB 5/13 + 4 missing 永久非 scope
      ...
      驗證: MISSION.md R181 新欄補上 + 保留 R144 舊欄 (歷史基準不抹)

- [ ] **T-MKRA2: 護衛 test ship** — `scripts/k0_target_baseline_check.py` + pytest 5 case
      ↪ 對應 MCAP-2 + spec.md KAP-R2-S3 (走 R132/R167 模式)
      驗證: pytest 5/5 PASS + chain 20→20 守住 (走既 `render_prometheus_tests` mod 不擴張)

- [ ] **T-MKRA3: R181 接力順位 update** — 7 條卡死鏈 → 5 條 unblock, 2 條永久 skip
      ↪ 對應 MCAP-4
      驗證: engineering-log.md R182 entry + 接力順位 #1/#2 改 [永久 skip, OpenAB scope 移出 K0]

- [ ] **T-MKRA4: R181 提案整體 closure** — MISSION patch + 護衛 + log entry 一次 ship
      ↪ 對應 K40 spec closure
...
- [ ] **T-MKRB6: R181 提案整體 closure** — design + spec + module + handlers + 護衛一次 ship
...
- R181 PUA scope = T-MKR1~T-MKR2 + 護衛 test (T-MKRA2 走既 mod, chain 20→20 守住, 不算 T-MKRA1~4 整體)
...
- 不選風險: K0 永久 4/13 + 1/13 + 9/13, 接力順位永久卡 5 條, MISSION 失靈
